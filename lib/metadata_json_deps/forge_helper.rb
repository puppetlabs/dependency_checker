...
class MetadataJsonDeps::ForgeHelper
  ...
  def get_current_version(module_name)
    ...
    version = nil
    version = SemanticPuppet::Version.parse(@cache[module_name]['current_release']['version']) if @cache.key?(module_name)
  ...
  def get_module_data(module_name)
    ...
    @cache[module_name] = module_data = get_module_data_from_forge(module_name) unless module_data
